...
os.environ['ROS_MASTER_URI'] = 'http://127.0.0.1:11324'
...
LOG = ROOT/'logs/navigation_mission_isolated'
...
try:
    with socket.socket() as probe:
        if probe.connect_ex(('127.0.0.1', 11324)) == 0:
            raise RuntimeError('test port11324 already in use; refusing to share a master')
    launch(['roscore', '-p', '11324'], 'master')
    ...
    while True:
        ...
        time.sleep(.1)
    rospy.init_node('navigation_mission_harness', disable_signals=True)
    pubs = {
    ...
    nav=launch(['roslaunch','--skip-log-check','ducted_bringup','local_avoidance.launch'], 'navigation_default')
    ...
    nav=launch(['roslaunch','--skip-log-check','ducted_bringup','local_avoidance.launch',
