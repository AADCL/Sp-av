...
LOG = ROOT/'logs/global_local_mission_isolated'
...
data = dict(x=0., y=0., z=1.2, yaw=0., mode='POSCTL', rc_mode='command',
            ...
            obstacle=False, follow=True, mode_calls=[], mission_history=[], telemetry_history=[],
...
try:
    ...
    while True:
        ...
        time.sleep(.1)
    rospy.init_node('global_local_mission_harness', disable_signals=True)
    import tf2_ros
    from geometry_msgs.msg import TransformStamped
    broadcaster=tf2_ros.StaticTransformBroadcaster()
    transform=TransformStamped();transform.header.stamp=rospy.Time.now()
    transform.header.frame_id='odom';transform.child_frame_id='map';transform.transform.rotation.w=1.
    broadcaster.sendTransform(transform)
    points=[((x+.5)*.2,(y+.5)*.2,(z+.5)*.2,-3.5)
            for x in range(-25,25) for y in range(-25,25) for z in range(-5,20)]
    occupancy=LOG/'observed_occupancy.pcd'
    with occupancy.open('w') as f:
        f.write('VERSION 0.7\nFIELDS x y z intensity\nSIZE 4 4 4 4\nTYPE F F F F\nCOUNT 1 1 1 1\nWIDTH %d\nHEIGHT 1\nPOINTS %d\nDATA ascii\n'%(len(points),len(points)))
        for p in points:f.write('%f %f %f %f\n'%p)
    (LOG/'mapping_metadata.yaml').write_text('format_version: 1\nframe_id: map\noccupancy_resolution: 0.2\n')
    pubs = {
    ...
    nav=launch(['roslaunch','--skip-log-check','ducted_bringup','global_local_planning.launch','occupancy_file:='+str(occupancy)], 'navigation_default')
    ...
    nav=launch(['roslaunch','--skip-log-check','ducted_bringup','global_local_planning.launch','occupancy_file:='+str(occupancy),
