      ...
      current_scan_ptr_(new pcl::PointCloud<pcl::PointXYZI>) 
{
    ...
    if (map_path_.empty()) {
        throw std::runtime_error("globalmap_dir must name a PCD map file");
    }
    if (map_path_.front() != '/') {
        map_path_ = std::string(std::getenv("HOME")) + "/" + map_path_;
    }
...
bool Matching::InitGlobalMap() {
    const int load_result = pcl::io::loadPCDFile(map_path_, *global_map_ptr_);
    if (load_result < 0 || global_map_ptr_->empty()) {
        throw std::runtime_error("failed to load non-empty global map: " + map_path_);
    }
