...
namespace esekfom
{
	...
	public:
		...
		{
			// 计算加速度
			acc_ = i_in.acc - x_.ba;   	//测量加速度 = a_m - bias
...
#pragma omp parallel for
			...
			for (int i = 0; i < effct_feat_num; i++)
			{
				...
				V3D A(point_I_crossmat * C);//这个point_I_crossmat应该也要转置一下的吧？疑问
				if (extrinsic_est)
				{
		 ...
		 * 
		 * @param R 观测噪声
		 * @param reg_pose 点云匹配的位姿（imu系下的点云匹配）
		 * @return 
		 */
		void update_with_reg(double R,Eigen::Matrix4d & reg_pose){
			cov P_updated = P_;
			...
			//观测噪声
        	Eigen::Matrix<double,6,6> V = Eigen::Matrix<double,6,6>::Identity()*R;
			//卡尔曼增益
