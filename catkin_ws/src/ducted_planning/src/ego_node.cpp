...
class EgoNode {
  ros::NodeHandle nh_{"~"}; ros::ServiceServer service_; ros::Publisher path_pub_;
  GridMap::Ptr environment_{new GridMap};
  ego_planner::BsplineOptimizer optimizer_;
  double max_speed_,max_acceleration_,vertical_speed_,horizon_;
public:
  EgoNode(){
  ...
  bool plan(ducted_planning::PlanLocal::Request& req,ducted_planning::PlanLocal::Response& res){
    ...
    try{
      ...
      if(req.header.frame_id!="odom"||req.obstacles.header.frame_id!="odom"||
         ...
         !std::isfinite(age)||age<-.05||age>.4||!start.allFinite()||!goal.allFinite()||
        ...
        throw std::runtime_error("invalid or stale EGO snapshot");
      if(start.z()<req.minimum_z||start.z()>req.maximum_z||goal.z()<req.minimum_z||goal.z()>req.maximum_z)
        throw std::runtime_error("absolute target violates measured AGL envelope");
      ...
      if(environment_->getInflateOccupancy(start))throw std::runtime_error("current body intersects local occupancy");
      ...
      if(environment_->getInflateOccupancy(target))throw std::runtime_error("local endpoint is occupied; holding for new observation or waypoint");
      std::vector<V> local;
      if(distance<.04 && velocity.norm()<.05){
        ...
        for(int i=0;i<=steps;++i){
          ...
          if(!p.allFinite()||!environment_->segment(previous,p))
            throw std::runtime_error("EGO curve failed full collision/AGL/horizon validation");
      ...
      if(environment_->expired()||std::chrono::duration<double>(std::chrono::steady_clock::now()-began).count()>.27||
         (ros::Time::now()-req.header.stamp).toSec()>.45)
    ...
    }catch(const std::exception& e){res.success=false;res.reason=e.what();res.local_path.poses.clear();}
    return true;
  }
};
}
int main(int argc,char** argv){ros::init(argc,argv,"ego_local_planner");
