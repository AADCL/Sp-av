...
// Unused upstream gradient-descent header omitted; EGO uses L-BFGS.
// using namespace std;
...
namespace ego_planner
{
  ...
  std::vector<std::vector<Eigen::Vector3d>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
  {
    ...
    int i_end = (int)init_points.cols() - order_; // Ducted integration: segment the complete local seed.
    ...
    for (size_t i = 0; i < segment_ids.size(); i++)
    {
      ...
      for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j)
      {
        ...
        if (got_intersection_id == j && intersection_point.allFinite())
        {
      ...
      //step 3
      if (got_intersection_id >= 0 && !cps_.direction[got_intersection_id].empty())
      {
  ...
  int BsplineOptimizer::earlyExit(void *func_data, const double *x, const double *g, const double fx, const double xnorm, const double gnorm, const double step, int n, int k, int ls)
  {
    ...
    return opt->grid_map_->expired() || (opt->force_stop_type_ == STOP_FOR_ERROR || opt->force_stop_type_ == STOP_FOR_REBOUND);
  }
  ...
  bool BsplineOptimizer::rebound_optimize()
  {
    ...
    do
    {
          ...
          result == lbfgs::LBFGS_STOP)
      {
        ...
        for (double t = tm; t < tmp; t += t_step) // Ducted integration: validate the complete local trajectory.
        {
